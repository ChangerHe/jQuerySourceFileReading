...
(function(window, undefined) {
    ...
    // A central reference to the root jQuery(document)
        ...
        // Map over the $ in case of overwrite  主要的作用还是避免冲突
        _$ = window.$,

        // [[Class]] -> type pairs 提供类型的存储
        class2type = {},

        // List of deleted data cache ids, so we can reuse them
        // 老版本的数据缓存有关系,2.0+版本没神勇了
        core_deletedIds = [],
        ...
        // jQuery的总接口
        // 其实在这里可能很多人会疑惑不解,因为我选择器就好好地做一个选择器啊,还搞个context做什么.当然,我百度了一下这个问题,这其实是一个执行的上下文环境(context的英文释义即是如此),这个上下文环境时一个可选的参数,在我们一般的开发环境中其实是不需要的,因为我们在页面中仅一个框架的时候,执行上下文就是document.
        // 但是有一种情况是例外的,当我们的页面中包含多个frame的时候,我们就需要指定执行的上下文环境了(contentWindow.document),讲真这个东西有什么卵用吗?
        jQuery = function(selector, context) {
            // The jQuery object is actually just the init constructor 'enhanced'
            // rootjQuery = jQuery(document);其实rootjQuery就是jQuery的入口,或者说是根节点
            return new jQuery.fn.init(selector, context, rootjQuery);
            // 数组返回的是一个对象,这样才能有相应的对象的方法
            // jQuery.fn = jQuery.prototype 所以这个时候两者是对等的
            // jQuery.prototype.init.prototype = jQuery.prototype将构造函数的方法的原型赋予给了init的原型,这样就形成了一个链式的调用
            // init()其实就相当于直接初始化了这个方法,就不用$().这样调用了
        },
        ...
        // 注意当我们进行cssHACK的时候,会出现这样的情况,对于其他浏览器,我们使用时第一个字母是小写的,但是IE的第一个字母是要大写的
        /*
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        // 就是一个简单的版本号,版本号前面已经定义了
        jquery: core_version,

        // 将构造函数指向自身Aaa.prototype.constructor = Aaa
        // 但是如果jq在这个时候因为使用了原型方法,间接将constructor改了,被改向Object,所以要改回来
        constructor: jQuery,
        // 主要的选择器的接口
        init: function(selector, context, rootjQuery) {
            ...
            // 如果没有传值,或传了上面的几个值,就直接返回了,返回这个对象,也就是jQuery本身
            if (!selector) {
                return this;
            }

            // Handle HTML strings
            // 如果选取元素为字符串,则进行条件判断操作
            if (typeof selector === "string") {
                // 匹配一个左右有尖括号,中间至少一个长度字符串的标签,也就是一个完整的html标签结构
                // 也就是说,当我们在$('')中传入标签的时候,从这里进入
                if (selector.charAt(0) === "<" && selector.charAt(selector.length - 1) === ">" && selector.length >= 3) {
                    // Assume that strings that start and end with <> are HTML and skip the regex check
                    // 定义的match作为一个数组
                    // 问题是,为什么match要在两侧传一个空值进去呢?
                    match = [null, selector, null];

                } else {
                    // 否则,使用exec()方法对其进行重新匹配,其实exec()匹配出来的结果是和match是一样的,只不过匹配的方式不同,exec()匹配的元素放在括号内,match()匹配的元素放在括号外而已,最后生成的是一个伪数组,数组具有长度,可以像getElements方法一样可以被for方法进行长度的遍历
                    match = rquickExpr.exec(selector);
                }

                // Match html or make sure no context is specified for #id
                // 这里是匹配的一个当传入的值是#id的情况下的接口,因为match[1]匹配的是(<[\w\W]+>),也就是标签,当这个为undefined时,则表示并没有匹配到这个标签.也就是说,我们匹配到的是第二个(因为没有匹配到的话,match会为undefined,直接执行else的操作了)
                if (match && (match[1] || !context)) {
                    ...
                    if (match[1]) {
                        ...
                        // 关于merge的用法,ctrl+f搜索,得到相应方法
                        // 将传入的元素解析为数组,一个可以被直接翻译为HTML文档的数组,当我们调用append或者appendTo的时候,可以直接加载到文档流中,you see ,parseHTML传入三个参数,第一个参数是HTML的文档,第二个参数是执行的上下文,第三个参数是是否执行script脚本的匹配,这里会考虑到安全性的问题,当然,第三个参数默认是false,就是不匹配,但是我们下面的merge是启用了这个方法的,这一点要注意.
                        // 大家查看到parseHTML这个方法的时候,其实它还做了一个很人性化的操作,当我们没有传入第二个参数的时候,不用为第二个参数留空,直接在第二个参数上面写上布尔值(第三个参数),这样也是可以的.
                        // merge类似于ES5数组的concat方法,可能是因为考虑到兼容性,所以自己定义了一个merge方法,就是连接两个字符串,并将第一个字符串返回
                        jQuery.merge(this, jQuery.parseHTML(
                            // 匹配到的相应HTML字符串
                            match[1],
                            // 执行上下文,当有传入context而且context包含nodeType属性,表示一定是一个dom对象,,则返回context.ownerDocument,如果不是标准的dom对象,也就是不返回正确的1-12的nodetype值的时候,则默认为document,这样的操作是为了保护整体的文档流能够正确的执行,因为当你输错了你的iframe这些字符的时候,jQuery可以默认匹配到本身文档流的document.
                            context && context.nodeType ? context.ownerDocument || context : document,
                            // 表示是否解析<script></script>标签,默认为false,这里设置成了true
                            true
                        ));

                        // HANDLE: $(html, props)
                        // rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>|)$/正则表达式看起来很复杂,实际上就是匹配了一个完整的非单标签,也就是有结束标签的标签,注意后面的\1,指代前面的第一个括号内容
                        // 将执行上下文传入isPlainObject中进行判断,是为了判断它是不是Object类型,或者是DOM节点,或者是window
                        // match是之前定义的字符串,当匹配的字符串有<>符号时,等于 [null, selector, null] 
                        // 其实它的作用就是匹配后面的json,并将json作为前面标签的属性加进去
                        // 具体的底层实现还有点懵逼,等看了下面的再更
                        if (rsingleTag.test(match[1]) && jQuery.isPlainObject(context)) {
                            // 为什么要这样进行遍历呢?
                            for (match in context) {
                                // Properties of context are called as methods if possible
                                // 如果可能,context的变量将作为方法
                        ...
                        // 使用elem变量对匹配出的'#'后面的字符串进行选取
                        elem = document.getElementById(match[2]);

                        // Check parentNode to catch when Blackberry 4.6 returns
                        // nodes that are no longer in the document #6963
                        // 如果elem能够匹配到,而且elem存在父节点,另外是对于黑莓系统的一个问题修复,这个里面是存在BUG的.
                        if (elem && elem.parentNode) {
                            // Inject the element directly into the jQuery object
                            // 以下为具体的黑莓手机的问题修复,因为使用 document.createDocumentFragement 创建文档结构的时候,存在这个节点树在插入文档流且使用removeChild移除以后,还能够被检索到的问题,所以我们在检索elem之后,会再判断一下其是否有父节点.以确认是否该节点确实存在于文档流中
                            this.length = 1;
                            this[0] = elem;
                        }

                        // 在这个时候,既然elem是document中的id,那其执行上下文则必然为document
                        this.context = document;
                        // 这两个绑定的方法其实很像小红书里面的构造函数模式和原型模式的组合使用方式,将值直接对其进行赋值,则每new出来的对象都有这样的属性,当然,是在判定为#的情况下
                        this.selector = selector;
                        // 返回本身,这其实是方便进行一种连缀的写法,当返回自身的时候,我们使用完这个函数之后,还可以在这个函数之后再次针对这个对象再执行其他的函数
                        return this;
                    }

                    // HANDLE: $(expr, $(...))
                    // 当无执行上下文的时候,
                    // rootjQuery = jQuery(document);其实rootjQuery就是jQuery的入口,或者说是根节点,没有执行上下文时,就是在根节点上查找到这个相应的选择器.
                } else if (!context || context.jquery) {
                    return (context || rootjQuery).find(selector);

                    // HANDLE: $(expr, context)
                    // (which is just equivalent to: $(context).find(expr)
                    // 下面是操作当存在选择器,和制定了执行上下文的情况
                } else {
                    // this.consructor被调用者的构造函数,是指向自身的,也就是相当于jQuery(context).find(selector)
                    // 问题是,它为什么要这样子做呢?
                ...
                // 如果选择器中传入了一个函数,它就自动转为$(document).ready(function)这样的方法了,上面也说了,这其实就是document ready的简写
            } else if (jQuery.isFunction(selector)) {
                return rootjQuery.ready(selector);
            }

            // 当选择器的选择器不为undefined的时候,为什么要这样做呢?
            // 这个其实是为了照顾那些写错的童鞋,当我们使用选择器的时候,可能会写错成这样: $($('#div')),这个时候jQuery不会报错,还是会返回你想要的值,这其实就是这个地方做到的
            if (selector.selector !== undefined) {
                this.selector = selector.selector;
                this.context = selector.context;
            }

            // makeArray的作用是将类数组对象转化为真正的数组
            // 可以看到,makeArray是传递了第二个this作为参数的,这个情况下其实
            // 我还没懂!
        ...
        // 上面说的很清楚了,在匹配到的element中选取第n个元素,然后将选取的元素作为一个新的数组
        get: function(num) {
            return num == null ?
    ...
    jQuery.extend({
        ...
        isPlainObject: function(obj) {
            ...
            // - DOM nodes
            // - window
            // 如果传入的值不是Object类型(不是对象),也不是dom节点类型,也不是window,则返回一个false
    ...
    */
    ...
    jQuery.fn.extend({
        // find方法,其实就是一种复杂选择器的实现
        // find方法目的在于查找需要的相应元素,具体等看到这里在更
        find: function(selector) {
            var i,
                ret = [],
                self = this,
                len = self.length;
            // 当然,首先要判断选择器是否为字符串类型,不是的话当然是不能够匹配的了
            if (typeof selector !== "string") {
